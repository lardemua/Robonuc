/**
*      @file  
*      @brief  Program to process a point cloud acquired by the kinect and detect simple objects (adapted from Joana Mota 2018)
*
...
#include "ros/ros.h"
#include "bin_picking/get_targets_pose.h"
...
void cloud_cb(const PointCloud::ConstPtr &cloud_input)
{
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = ece_indices.begin(); it != ece_indices.end(); ++it)
  {
    ...
    // viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, pointcloudCentroidSurf);

    // // Visualization of the the normal on the centroid of every object
    // viewer->addPointCloudNormals<pcl::PointXYZRGB, pcl::Normal> (centroid_ptr_real, normal_centroid_ptr,100,0.1, pointcloudNormal);
...
bool service_cb(bin_picking::get_targets_pose::Request &req,
...
int main(int argc, char **argv)
{
  ...
  ros::ServiceServer service = nh.advertiseService("get_targets_pose", service_cb);
  service_request=false;
