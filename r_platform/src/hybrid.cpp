 ...
 *
 *
 *      @author   Tiago Tavares tiagoatavares@ua.pt Adapt from (Luís Sarmento - sarmento@ua.pt)
 *
 *    @internal
...

  {
    ...
    if (l_vel < 0 || a_vel != 0)
    {
           ...
           i++)
      {
        // ROS_INFO("i=%d", i);
        if (laser[i] <
                (abs(l_vel) * 0.15 + 0.068 * l_vel * l_vel) * 5.0f + 0.2 &&
            laser[i] > 0.1)
        {
          n_danger++;
          ROS_INFO("Zona 1a - Danger");
      ...
      {
        if (laser[i] <
                0.37 + (abs(l_vel) * 0.15 + 0.068 * l_vel * l_vel) * 2.0 &&
            laser[i] > 0.1)
        {
          n_danger++;
          ROS_INFO("Zona 2a - Danger");
      ...
      {
        for (int i =
                 TeleopRobonuc::rad2LaserCoord(1.0 * M_PI / 2.0, size_laser);
             i < TeleopRobonuc::rad2LaserCoord(3.0 * M_PI / 4.0, size_laser);
             i++)
      ...
      if (a_vel < 0)
      {
        for (int i =
                 TeleopRobonuc::rad2LaserCoord(-3.0 * M_PI / 4.0, size_laser);
             i < TeleopRobonuc::rad2LaserCoord(-1.0 * M_PI / 2.0, size_laser);
             i++)
        {
          if (laser[i] <
                  (0.37 + 1.5 * alpha) /
    ...
    if (n_danger > 5)
    {
      laser_danger_2_ = true;

    } // obstáculo na direção do movimento
    else
    {
      laser_danger_2_ = false;
    }
  }
}
...
void TeleopRobonuc::modeDecider(void)
{
  ...
  if (vel_msg_.linear_vel != 0 && joy_l_vel_ == 0)
  {
  ...
  if (mode_ == 2) // automatic
  {
    ...
    {
      vel_msg_.linear_vel = vel_msg_.linear_vel / 5;
    }
    else
    {
  ...
  {
    if (!laser_danger_ && !laser_danger_2_)
    {
    ...
    else
    {
      ...
      marker.color.b = 1.0;
      marker.text = "AUTOMATIC";
      if (auto_l_vel_ != 0 || auto_a_vel_ != 0)
      {
        time = 10;
      }
