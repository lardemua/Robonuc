...
def generate_plan_sinosoide(group, amplitude, sinosoide_number, q):
    ...
    starting_point = Pose() #= upperpoint
    starting_point = group.get_current_pose().pose
    waypoints.append(copy.deepcopy(starting_point))


    final_point = Vector3()
    final_point.x = starting_point.position.x
    final_point.y = starting_point.position.y
    final_point.z = starting_point.position.z - amplitude
    ...
    for n in range(1, number_of_points+1):
        ...
        point.position.x = final_point.x + n*(length/3) * vector.x
        point.position.y = final_point.y + n*(length/3) * vector.y
        point.position.z = final_point.z + n*(length/3) * vector.z
    ...
    final_position.orientation.z = q[2]
    final_position.orientation.w = q[3]
    final_position.position.x = starting_point.position.x
    final_position.position.y = starting_point.position.y
    final_position.position.z = starting_point.position.z

    waypoints.append(copy.deepcopy(final_position))

    #------------------------------------------
    #waypoints.append(copy.deepcopy(waypoints))
    
    for n in range(1, sinosoide_number*len(waypoints)):
        waypoints.append(copy.deepcopy(waypoints[n]))
    #--------------------------------------------
